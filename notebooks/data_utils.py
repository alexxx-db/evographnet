...
def cross_val_indices(folds, num_samples, new=False):
    ...
    try:
        if new == True:
            raise IOError
        with open("/Workspace/Shared/EvoGraphNet/data/" + str(folds) + "_" + str(num_samples) + "cv_train", "rb") as f:
            train_indices = pickle.load(f)
        with open("/Workspace/Shared/EvoGraphNet/data/" + str(folds) + "_" + str(num_samples) + "cv_val", "rb") as f:
            val_indices = pickle.load(f)
    except IOError:
        for tr_index, val_index in kf.split(np.zeros((num_samples, 1))):
            train_indices.append(tr_index)
            val_indices.append(val_index)
        with open("/Workspace/Shared/EvoGraphNet/data/" + str(folds) + "_" + str(num_samples) + "cv_train", "wb") as f:
            pickle.dump(train_indices, f)
        with open("/Workspace/Shared/EvoGraphNet/data/" + str(folds) + "_" + str(num_samples) + "cv_val", "wb") as f:
