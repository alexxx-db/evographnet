...
class MRDataset(InMemoryDataset):
    ...
    def data_read(self, h="lh", nbr_of_subs=55, connectomes=1):   #### 54 subjects
        """
        ...
        # data_path = "../data"
        # data_path = "../DATA_prep/data_Tau_long"
        # data_path = "/dbfs/mnt/POC-data-science/DPM_DL/EvoGraphNet/DATA_prep/data_Tau_long"
        data_path = "/dbfs/mnt/ml-dev-sandbox/data/hls/dpm_dl/evographnet/DATA_prep/data_Tau_long"
        for i in range(1, nbr_of_subs):
            ...
            for mr in ["MR1", "MR2"]:
                ...
                ### default: n_roi = 35 (Desikan-Killiany atlas)
                read_limit = (connectomes * n_roi)
                t_stacked = np.vstack((t0[:read_limit, :], t1[:read_limit, :], t2[:read_limit, :]))
                tsr = t_stacked.reshape(3, connectomes * n_roi, n_roi)
        ...
        final_views = torch.tensor(np.moveaxis(subs.reshape(-1, 3, (connectomes * n_roi), n_roi), 1, -1), dtype=torch.float)
        return final_views
    @property
...
def cross_val_indices(folds, num_samples, new=False):
    ...
    try:
        if new == True:
            raise IOError
        with open("/dbfs/mnt/ml-dev-sandbox/data/hls/dpm_dl/data/" + str(folds) + "_" + str(num_samples) + "cv_train", "rb") as f:
            train_indices = pickle.load(f)
        with open("/dbfs/mnt/ml-dev-sandbox/data/hls/dpm_dl/data/" + str(folds) + "_" + str(num_samples) + "cv_val", "rb") as f:
            val_indices = pickle.load(f)
    except IOError:
        for tr_index, val_index in kf.split(np.zeros((num_samples, 1))):
            train_indices.append(tr_index)
            val_indices.append(val_index)
        with open("/dbfs/mnt/ml-dev-sandbox/data/hls/dpm_dl/data/" + str(folds) + "_" + str(num_samples) + "cv_train", "wb") as f:
            pickle.dump(train_indices, f)
        with open("/dbfs/mnt/ml-dev-sandbox/data/hls/dpm_dl/data/" + str(folds) + "_" + str(num_samples) + "cv_val", "wb") as f:
