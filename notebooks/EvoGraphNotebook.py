...
loss_dict = {"BCE": torch.nn.BCEWithLogitsLoss().to(device),
...
for fold in range(folds):
    ...
    ### use scaler to define mixed precision ###
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(h_data_test_loader):
            ...
            # Identity Loss
            # Inspect the input data - the error below suggests that more than 2GB data is being passed into this function at one time/row
            # general recommendations are to *Sparkify* this code
            # 1. break up this code line into multiple cells in a notebook
            # 2. examine the data sets being passed into each DL function (what's the size of the data being passed to identity_loss?)
            #    do this by using Dataframes
            # 3. 
